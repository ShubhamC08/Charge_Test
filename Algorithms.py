...
def main():
    ...
    Constant_Current_Charging(spi, conn)
...
def Perform_OCV(spi,conn):
    #OCV Block Start
    OCV_Command =[0x1,0x0,0x0]
    spi.writebytes(OCV_Command)
    Dummy_Read = spi.readbytes(3)
    time.sleep(0.05)
    Read_BMS_Command = [0x2,0x0,0x0]
...
def Read_BMS_Parameters(spi,conn):
...
def Update_Charging_Parameters(spi,Charging_Voltage,Charging_Current):
    Charge_V = list(Charging_Voltage.to_bytes(2,'big'))
    Charge_C = list(Charging_Current.to_bytes(2,'big'))
    Update_Voltage_Command = 0x3
    Update_Current_Command = 0x4
    Update_Voltage = Charge_V.insert(0,Update_Voltage_Command)
    Update_Current = Charge_V.insert(0,Update_Current_Command)
    #Update Voltage
...
def Start_Charging(spi):
    Start_Charging_Command = [0x5,0x0,0x0]
    spi.writebytes(Start_Charging_Command)
    D_Read = spi.readbytes(3)

def Stop_Charging(spi):
    Stop_Charging_Command = [0x6,0x0,0x0]
...
def Constant_Current_Charging(spi,conn):
...
def Boost_Charging(spi,conn):
    ...
    while Exit_Flag == 0: #Add time here too
        ...
        #End Block
        #Add delay for 5minsif __name__ == '__main__':
    main()
        time.sleep(300)
    #Stopped Charging
    if Critical_Alarm_Flag == 1:
